...
impl Language for Swedish {
    ...
    fn every_x_minutes(&self, s: u16) -> String { format!("var {}:e minut", s) }
    fn every_x_seconds(&self, s: u16) -> String { format!("var {}:e sekund", s) }
    fn every_x_hours(&self, s: u16) -> String { format!("var {}:e timme", s) }
    fn every_minute_of_every_x_hours(&self, s: u16) -> String { format!("Varje minut, var {}:e timme", s) }
    
    fn at_time(&self, time: &str) -> String { format!("Klockan {}", time) }
    fn at_time_and_every_x_seconds(&self, time: &str, step: u16) -> String { format!("Klockan {}, var {}:e sekund", time, step) }
